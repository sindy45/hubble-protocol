...
def tweak_price(A: uint256, gamma: uint256,
    ...
    if new_D == 0:
        ...
        D_unadjusted = Math(self.math).newton_D(A, gamma, _xp)
    ...
    if p_i > 0:
        ...
        __xp[0] += dx_price
        for k in range(N_COINS-1):
            last_prices[k] = price_scale[k] * dx_price / (_xp[k+1] - Math(self.math).newton_y(A, gamma, __xp, D_unadjusted, k+1))
...
def exchange(i: uint256, j: uint256, dx: uint256, min_dy: uint256) -> uint256:
    ...
    if True:  # scope to reduce size of memory when making internal calls later
        ...
        dy = xp[j] - Math(self.math).newton_y(A, gamma, xp, self.D, j)
...
def _calc_withdraw_one_coin(A: uint256, gamma: uint256, token_amount: uint256, i: uint256,
    ...
    y: uint256 = Math(self.math).newton_y(A, gamma, xp, D, i)
